...
LOG_FILE="${LOG_DIR}/timezone_$(date +%Y-%m-%d_%H-%M-%S).log"
mkdir -p "$LOG_DIR"
...
# Función: Configurar timezone en una máquina
set_timezone() {
  local vmid="$1"
  local name="$2"
  local ip
  ...
  echo -e "${AZUL}[i] Configurando timezone en '${name}' (ID: ${vmid}, IP: ${ip})...${RESET}"
  ...
  result=$(ssh -n \
      ...
      -o BatchMode=yes \
      "root@${ip}" \
      "timedatectl set-timezone ${TIMEZONE} && timedatectl" 2>&1)

  if [[ $? -eq 0 ]]; then
    echo -e "${VERDE}[+] '${name}' timezone configurado correctamente.${RESET}"
    echo "$result" | grep -E "Time zone|Local time"
  else
    echo -e "${ROJO}[-] Error en '${name}'. Continuando...${RESET}"
    echo "$result"
  fi
}
...
# Función: Configurar en una máquina concreta por VMID
set_one() {
  local target_vmid="$1"
  local name
...
echo ""
echo "========================================="
echo "   Configuración de timezone en Proxmox  "
echo "   $(date '+%Y-%m-%d %H:%M:%S')          "
echo "========================================="
echo ""
echo "[i] Timezone objetivo: ${TIMEZONE}"

if [[ $# -eq 0 ]]; then
  echo -e "${ROJO}[-] Uso: $0 <vmid> | all${RESET}"
  echo -e "${AZUL}[i]   $0 2010       -> configura timezone en máquina con ID 2010${RESET}"
  echo -e "${AZUL}[i]   $0 all        -> configura timezone en todas las máquinas${RESET}"
  exit 1
fi
